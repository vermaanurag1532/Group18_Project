...
const char* udpAddress = "192.168.127.233"; // IP address of your computer server
const int udpPort = 8080;  // Port on which your server is listening
...
HardwareSerial LidarSerial(1); // TX 17, RX 16
...
void setup() {
  ...
  // Assign TX, RX pins
  LidarSerial.begin(baud_rate, SERIAL_8N1, 3);
...
void lidar_scan_point_callback(float angle_deg, float distance_mm, float quality,
  ...
  if ((i++ % 20 == 0) || scan_completed) {
    String lidarData = String(i) + " " + String(angle_deg) + " " + String(distance_mm) + "\n";
